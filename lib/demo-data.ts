...
function monthPayment(tenantId: string, propertyId: string, label: string, amount: number, monthsAgo: number, dueDay: number) {
  ...
  return {
    ...
    receiptNumber: `LF-${new Date(dueDate).toISOString().slice(0, 10).replace(/-/g, "")}-${tenantId.slice(-4).toUpperCase()}`,
    dueDate,
  ...
  return {
    ...
    payments: [
      ...
      monthPayment("tenant-dana", "property-maple", "Maple Street rent", 1650, 1, 1),
      {
        ...
        label: `Maple Street ${currentMonthName()} rent`,
        amount: 1650,
        baseAmountCents: 165000,
        lateFeeCents: 5000,
      ...
      monthPayment("tenant-evan", "property-cedar", "Cedar Avenue rent", 1050, 1, 5),
      {
        ...
        label: `Cedar Avenue ${currentMonthName()} rent`,
        amount: 1050,
        baseAmountCents: 105000,
        lateFeeCents: 0,
        paymentMethod: "ach",
        stripePaymentIntentId: "pi_demo_autopay_evan",
        dueDate: isoAtDayOffset(0),
        status: "pending",
      },
      ...
      monthPayment("tenant-nina", "property-cedar", "Cedar Avenue rent", 1050, 1, 5),
      {
        ...
        paymentMethod: "card",
        stripePaymentIntentId: "pi_demo_nina_paid",
        receiptNumber: `LF-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}-NINA`,
    ...
    paymentProfiles: [
      {
        id: "profile-dana",
        tenantId: "tenant-dana",
        stripeCustomerId: "cus_demo_dana",
        autopayEnabled: false,
        autopayMethod: "ach",
        savedPaymentLabel: "Primary checking ending in 4831",
        notificationChannels: ["email"],
        createdAt: isoAtMonthOffset(-4, 1),
        updatedAt: isoAtDayOffset(-2),
      },
      {
        id: "profile-evan",
        tenantId: "tenant-evan",
        stripeCustomerId: "cus_demo_evan",
        autopayEnabled: true,
        autopayMethod: "ach",
        savedPaymentLabel: "Business checking ending in 2207",
        notificationChannels: ["email"],
        createdAt: isoAtMonthOffset(-3, 5),
        updatedAt: isoAtDayOffset(-1),
      },
      {
        id: "profile-nina",
        tenantId: "tenant-nina",
        stripeCustomerId: "cus_demo_nina",
        autopayEnabled: true,
        autopayMethod: "card",
        savedPaymentLabel: "Visa ending in 4242",
        notificationChannels: ["email"],
        createdAt: isoAtMonthOffset(-3, 5),
        updatedAt: isoAtDayOffset(-1),
      },
    ...
    paymentEvents: [
      {
        id: "payment-event-1",
        rentPaymentId: "payment-evan-current",
        stripeEventId: "evt_demo_processing",
        eventType: "payment_intent.processing",
        payload: {
          paymentIntentId: "pi_demo_autopay_evan",
          amountCents: 105000,
          method: "ach",
        },
        createdAt: isoAtDayOffset(0, 7),
      },
      {
        id: "payment-event-2",
        rentPaymentId: "payment-nina-current",
        stripeEventId: "evt_demo_succeeded",
        eventType: "payment_intent.succeeded",
        payload: {
          paymentIntentId: "pi_demo_nina_paid",
          amountCents: 105000,
          method: "card",
        },
        createdAt: isoAtDayOffset(-1, 14),
      },
    ],
    requests: [
      {
